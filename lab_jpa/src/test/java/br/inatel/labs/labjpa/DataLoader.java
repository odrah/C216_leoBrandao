...
class DataLoader {
	...
	void load() {
		...
		Fornecedor f1 = new Fornecedor("Gasometro Madeira");
		...
		NotaCompraItem item1 = new NotaCompraItem(nc1, p1, new BigDecimal("300.00"), 2);
		NotaCompraItem item2 = new NotaCompraItem(nc1, p2, new BigDecimal("100.00"), 1);
		NotaCompraItem item3 = new NotaCompraItem(nc1, p3, new BigDecimal("500.00"), 3);

		item1 = notaCompraService.salvarNotaCompraItem(item1);
		item2 = notaCompraService.salvarNotaCompraItem(item2);
		item3 = notaCompraService.salvarNotaCompraItem(item3);

		NotaCompraItem item4 = new NotaCompraItem(nc2, p4, new BigDecimal("400.00"), 7);
		NotaCompraItem item5 = new NotaCompraItem(nc2, p2, new BigDecimal("100.00"), 2);
		NotaCompraItem item6 = new NotaCompraItem(nc2, p5, new BigDecimal("700.00"), 1);

		item4 = notaCompraService.salvarNotaCompraItem(item4);
		item5 = notaCompraService.salvarNotaCompraItem(item5);
		item6 = notaCompraService.salvarNotaCompraItem(item6);
